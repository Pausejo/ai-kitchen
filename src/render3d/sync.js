...
export function syncScene(state, hint) {
  ...
  reconcile(scene, t, maps.stations, state.stations, (s) => s.id, makeStationGroup, (g, s) =>
    updateStation(g, s, state, hint, t, dt, draining),
  );
...
function updateStation(g, s, state, hint, t, dt, draining) {
  const ud = g.userData;

  // Las placas de nombre son ayuda del tutorial: al terminarlo se desvanecen
  // y la cocina queda limpia (el mueble habla por sí solo).
  const tutorialOn = state.learningPhase || state.subagentLearningPhase;
  const pm = ud.plate.material;
  pm.opacity = damp(pm.opacity, tutorialOn ? 1 : 0, 4, dt);
  ud.plate.visible = pm.opacity > 0.02;
